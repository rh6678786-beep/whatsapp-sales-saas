...
export class AILearningService {
  ...
  static async initialize(adminId: string = 'default-admin') {
    ...
    console.log(`🧠 AI Learning System initializing for ${adminId}...`);
    
    const settings = await dbService.getSettings(adminId);
    const savedPatterns = settings.aiLearningPatterns || [];
    const patterns = this.getPatternsForAdmin(adminId);
    
    savedPatterns.forEach((p: ResponsePattern) => {
      patterns.set(p.id, p);
    });
    ...
    console.log(`🧠 AI Learning loaded ${patterns.size} patterns for ${adminId}`);
  }
  ...
  static async analyzeChat(
    ...
    for (const keyword of keywords) {
      ...
      if (pattern) {
        ...
        pattern.avgLeadScore = ((pattern.avgLeadScore * (pattern.totalUses - 1)) + leadScore) / pattern.totalUses;
  ...
  static async markSaleClosed(userId: string, adminId: string = 'default-admin'): Promise<void> {
    ...
    await this.savePatterns(adminId);
    console.log(`🎉 Sale closed for user: ${userId}. Learning patterns updated for ${adminId}.`);
  }

  static async markSaleFailed(userId: string): Promise<void> {
    console.log(`❌ Sale failed for user: ${userId}.`);
  }
  ...
  private static findMatchingPattern(patterns: Map<string, ResponsePattern>, keyword: string): ResponsePattern | undefined {
  ...
  private static async savePatterns(adminId: string = 'default-admin') {
    ...
    try {
      ...
      console.error(`[LEARNING] Failed to save patterns:`, e.message);
    }
  }
  ...
  static getPatternRecommendations(customerMessage: string, adminId: string = 'default-admin'): string[] {
    ...
    for (const keyword of keywords) {
      ...
      if (bestPattern && bestPattern.successRate >= 0.5) {
        recommendations.push(`Use "${bestPattern.pattern}" pattern (${Math.round(bestPattern.successRate * 100)}% success rate)`);
      }
    }
